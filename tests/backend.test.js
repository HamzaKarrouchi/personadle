...
describe('buildGameSession', () => {
  ...
  it('played_date matches YYYY-MM-DD format', () => {
    const session = buildGameSession({
      mode: 'emoji', targetName: 'Joker', result: 'giveup', attempts: 6,
    });
  ...
  it('rounds time_ms', () => {
    const session = buildGameSession({
      mode: 'music', targetName: 'Last Surprise', result: 'win',
      attempts: 1, timeMs: 12345.7,
    });
  ...
  it('defaults filters to empty array when not provided', () => {
    const session = buildGameSession({
      mode: 'silhouette', targetName: 'Aigis', result: 'win', attempts: 2,
    });
  ...
  it('defaults timeMs to 0 when not provided', () => {
    const session = buildGameSession({
      mode: 'personae', targetName: 'Elizabeth', result: 'giveup', attempts: 5,
    });
...
describe('savePendingSession — offline (no API)', () => {
  ...
  it('accumulates multiple sessions in order', async () => {
    const s1 = buildGameSession({ mode: 'classic',   targetName: 'Ryuji', result: 'win',    attempts: 3 });
    const s2 = buildGameSession({ mode: 'emoji',     targetName: 'Ann',   result: 'giveup', attempts: 6 });
    const s3 = buildGameSession({ mode: 'silhouette',targetName: 'Yusuke',result: 'win',    attempts: 1 });
  ...
  it('preserves existing sessions already in queue', async () => {
    ...
    localStorage.setItem('pendingSessions', JSON.stringify([
      { mode: 'music', target_name: 'Burn My Dread', result: 'win', attempts: 1, time_ms: 0, active_filters: [], played_date: '2026-04-09' }
    ]));

    const s = buildGameSession({ mode: 'classic', targetName: 'Fuuka', result: 'win', attempts: 4 });
...
describe('savePendingSession — online (API available)', () => {
  ...
  it('posts session to API and does NOT write to localStorage', async () => {
    ...
    const session = buildGameSession({
      mode: 'alloutattack', targetName: 'Shadow', result: 'win', attempts: 2,
    });
  ...
  it('falls back to localStorage if API call throws (offline-first)', async () => {
    ...
    const session = buildGameSession({
      mode: 'personae', targetName: 'Izanagi', result: 'win', attempts: 1,
    });
...
describe('migration payload construction', () => {
  ...
  it('correctly extracts profile and pendingSessions from localStorage', () => {
    const fakeProfile = {
      ...
      stats: {
        wins: 42, giveups: 5, games: 47,
        streak: 3, streakRecord: 7,
        totalTimeMinutes: 312, perfectWins: 4,
      },
    };

    const fakeSessions = [
      { mode: 'classic', played_date: '2026-04-09', target_name: 'Ryuji', result: 'win', attempts: 3, time_ms: 45000, active_filters: [] },
      { mode: 'emoji',   played_date: '2026-04-08', target_name: 'Ann',   result: 'win', attempts: 2, time_ms: 30000, active_filters: ['P5'] },
    ];
