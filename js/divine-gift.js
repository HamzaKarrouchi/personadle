...
export async function showGiftsFromIds(rawItems, title = '🎁 Gift received!') {
  ...
  const get    = url => fetch(`${prefix}${url}`, { credentials: 'include' })
  ...
  const bySlug  = arr => Object.fromEntries((Array.isArray(arr) ? arr : []).map(b => [b.slug, b]));
  ...
  const fmt = id => String(id).replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
...
function _startParticles(canvas) {
  ...
  (function draw() {
    ...
    particles.forEach(p => {
      p.y += p.vy; p.x += p.vx; p.alpha -= 0.0022;
      ...
      if (p.y > canvas.height || p.alpha <= 0) {
        p.y = -10; p.x = Math.random() * canvas.width; p.alpha = 0.6 + Math.random() * 0.4;
      }
    });
    requestAnimationFrame(draw);
  })();

  setTimeout(() => { running = false; }, 5500);
}
