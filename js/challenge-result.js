...
function showChallengeResult({
  ...
  const labelXp    = t('challenge_result.xp_gained',  '+{{xp}} XP Social Link').replace('{{xp}}', xpGained);
...
export async function checkChallengeCompletion(mode, myAttempts, isWin) {
  ...
  let challenge;
  try { challenge = JSON.parse(raw); } catch { return; }
  ...
  if (challenge.filterKey && challenge.originalFilters !== null && challenge.originalFilters !== undefined) {
  ...
  if (api && challenge.senderId) {
    ...
    fetches.push(
      api.socialLink.getByFriend(challenge.senderId)
...
export async function showSenderChallengeResult(msg) {
  ...
  if (api) {
    const [meProfileRes, receiverRes, slRes] = await Promise.allSettled([
      ...
      msg.receiver_id
        ? api.socialLink.getByFriend(msg.receiver_id)
            .then(({ link_id }) => api.socialLink.get(link_id))
            .catch(() => null)
