...
function _populate(overlay, data, t) {
  ...
  meAv.onerror = () => { meAv.src = '../img/default_avatar.png'; };
  ...
  frAv.onerror = () => { frAv.src = '../img/default_avatar.png'; };
  ...
  setTimeout(() => {
    ...
    if (xp_gained > 0) {
      xpEl.textContent = t('compare.xp_gained', '+{{xp}} XP Social Link').replace('{{xp}}', xp_gained);
    ...
    if (cooldown_until) {
      ...
      cdEl.textContent = t('compare.cooldown', 'You can compare again in {{time}}').replace('{{time}}', diff);
